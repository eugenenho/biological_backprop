...
def simple_forward_prop(data, labels, hidden_dim, batch_size):
    ...
    W2 = xavier_mat_init(W2)
    W3 = xavier_array_init(W3)
    
    b1 = xavier_array_init(b1)
    b2 = xavier_array_init(b2)
    b3 = xavier_array_init(b3)
    ...
    for iter_num in range(m):
        ...
        z2 = np.dot(x1, W2) + b2
        h2  = sigmoid(z2)
        x2 = np.random.binomial(1, h2) # roll dice based on h

        z3 = np.dot(x2, W3) + b3
        h3 = sigmoid(z3)
        yHat = np.random.binomial(1, h3)
        ...
        for j in range(W3.shape[0]):
            W3[j] += row * reward * (yHat - h3) * x2[0, j] + lamb * row * (1 - reward) * (1 - yHat - h3) * x2[0, j]
            b3 += row * reward * (yHat - h3) * x2[0, j] + lamb * row * (1 - reward) * (1 - yHat - h3) * x2[0, j]

        # W2 update
        for i in range(W2.shape[0]):
            for j in range(W2.shape[1]):    
                W2[i, j] += row * reward * (x2[0, j] - h2[0, j]) * x1[0, i] + lamb * row * (1 - reward) * (1 - x2[0, j] - h2[0, j]) * x1[0, i]
                b2[j] += row * reward * (x2[0, j] - h2[0, j]) * x1[0, i] + lamb * row * (1 - reward) * (1 - x2[0, j] - h2[0, j]) * x1[0, i]
    ...
    plot_boundary(data, labels, W1, W2, W3, b1, b2, b3)
...
def net_predict(data, W1, W2, W3, b1, b2, b3):
    ...
    z2 = np.dot(x1, W2) + b2 # [m x n x n x h] = [m x h]
    h2  = sigmoid(z2)   # [m x h]
    x2 = np.int32(h2 >= 0.5) # [m x h]

    z3 = np.dot(x2, W3) + b3 # [m x h x h x 1]
    h3 = sigmoid(z3) # [m x 1]
    yHat = np.int32(h3 >= 0.5) # [m x 1]

    return yHat


def plot_boundary(X, Y, W1, W2, W3, b1, b2, b3):
    ...
    fig, ax = plt.subplots()
    Z = net_predict(np.c_[xx.ravel(), yy.ravel()], W1, W2, W3, b1, b2, b3)
